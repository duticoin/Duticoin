...
#if defined(NDEBUG)
# error "Duticoin cannot be compiled without assertions."
#endif
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            ...
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("Pre-allocating up to position 0x%x in dtc%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-dtc-length", false, "size limits failed");
    ...
    if (nSigOps > MAX_BLOCK_SIGOPS)
        return state.DoS(100, false, REJECT_INVALID, "bad-dtc-sigops", false, "out-of-bounds SigOpCount");
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL, bool fProofOfStake=true)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevdtc");
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevdtc");
...
void UnlinkPrunedFiles(std::set<int>& setFilesToPrune)
{
    for (set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        boost::filesystem::remove(GetBlockPosFilename(pos, "dtc"));
        boost::filesystem::remove(GetBlockPosFilename(pos, "rev"));
        LogPrintf("Prune: %s deleted dtc/rev (%05u)\n", __func__, *it);
    }
}
...
void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint("prune", "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d dtc/rev pairs\n",
...
FILE* OpenBlockFile(const CDiskBlockPos &pos, bool fReadOnly) {
    return OpenDiskFile(pos, "dtc", fReadOnly);
}
...
bool static LoadBlockIndexDB()
{
    ...
    // Check presence of dtc files
    LogPrintf("Checking all dtc files are present...\n");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile dtcdat(fileIn, 2*MAX_BLOCK_SIZE, MAX_BLOCK_SIZE+8, SER_DISK, CLIENT_VERSION);
        uint64_t nRewind = dtcdat.GetPos();
        while (!dtcdat.eof()) {
            boost::this_thread::interruption_point();

            dtcdat.SetPos(nRewind);
            nRewind++; // start one byte further next time, in case of failure
            dtcdat.SetLimit(); // remove former limit
            unsigned int nSize = 0;
            try {
                // locate a header
                unsigned char buf[MESSAGE_START_SIZE];
                dtcdat.FindByte(chainparams.MessageStart()[0]);
                nRewind = dtcdat.GetPos()+1;
                dtcdat >> FLATDATA(buf);
            ...
            try {
                // read block
                uint64_t nBlockPos = dtcdat.GetPos();
                if (dbp)
                    dbp->nPos = nBlockPos;
                dtcdat.SetLimit(nBlockPos + nSize);
                dtcdat.SetPos(nBlockPos);
                CBlock block;
                dtcdat >> block;
                nRewind = dtcdat.GetPos();
