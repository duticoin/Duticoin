...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-loadblock=<file>", _("Imports blocks from external dtc000??.dat file on startup"));
    ...
    strUsage += HelpMessageOpt("-reindex", _("Rebuild chain state and block index from the dtc*.dat files on disk"));
#ifndef WIN32
...
{
    const std::string URL_SOURCE_CODE = "<https://gitlab.com/duticoin/duticoin>";
    const std::string URL_WEBSITE = "<https://duticoin.com/>";
    ...
    return FormatParagraph(strprintf("Copyright (C) %i-%i The Bitcoin Core Developers", 2009, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Duticoin Developers", 2014, 2018)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Duticoin Core Developers", 2018, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Please contribute if you find Duticoin Core useful. "
...
void CleanupBlockRevFiles()
{
    ...
    // Glob all dtc?????.dat and rev?????.dat files from the blocks directory.
    // Remove the rev files immediately and insert the dtc file paths into an
    // ordered map keyed by block file index.
    LogPrintf("Removing unusable dtc?????.dat and rev?????.dat files for -reindex with -prune\n");
    ...
    for (directory_iterator it(blocksdir); it != directory_iterator(); it++) {
        ...
        {
            if (it->path().filename().string().substr(0,3) == "dtc")
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!boost::filesystem::exists(GetBlockPosFilename(pos, "dtc")))
                break; // No block files left to reindex
            FILE *file = OpenBlockFile(pos, true);
            if (!file)
                break; // This error is logged in OpenBlockFile
            LogPrintf("Reindexing block file dtc%05u.dat...\n", (unsigned int)nFile);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Duticoin Core version %s\n", FormatFullVersion());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Upgrading to 0.8; hard-link the old dtcnnnn.dat files into /blocks/
    ...
    if (!boost::filesystem::exists(blocksDir))
    {
        ...
        for (unsigned int i = 1; i < 10000; i++) {
            boost::filesystem::path source = GetDataDir() / strprintf("dtc%04u.dat", i);
            if (!boost::filesystem::exists(source)) break;
            boost::filesystem::path dest = blocksDir / strprintf("dtc%05u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking dtc%04u.dat: %s\n", i, e.what());
                break;
            }
        }
        if (linked)
