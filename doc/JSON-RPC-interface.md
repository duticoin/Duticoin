...
returned by RPCs that duticoin the mempool may not be up-to-date with the
...
The mempool state returned via an RPC duticoins all effects of mempool and chain
...
call is duticoined in the state of all wallet transactions. For example, if a
block contains transactions that conflicted with mempool transactions, the
wallet would duticoin the removal of these mempool transactions in the state.
...
this RPC may not yet be duticoined as such in this RPC response.
